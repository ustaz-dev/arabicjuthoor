...
> Auto-generated by `scripts/layer_2/merge_and_compute_770.py`. Every pair in the ≥0.65 pool was re-scored by Opus under the same calibrated Eye-2 prompt that Sonnet had: Juthoor thesis, polysemy rule, multi-step chains, method tags, calibration anchors from confirmed Tier-A cognates.

**Why this is the corrected headline number:** the original Sonnet bulk pipeline produced 770 pairs at ≥0.65. The IRR-150 audit on this same pool found Sonnet was lenient at the 0.65 boundary. This document replaces the IRR-150 estimate with the actual re-scoring of all 770. These are the calibrated numbers the dashboard should headline.
