#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 13 without committing or shipping.

Round 12 ended at OPEN-COMP-00533.  This round takes the next one hundred
...
import harvest_ancient_greek_round12 as R12  # noqa: E402


READING = R12.READING
REPORT = R12.REPORT
DATE = "2026-08-17"
FIRST_COMPLETION, LAST_COMPLETION, BATCH_SIZE = 534, 633, 50
EXPECTED_PREVIOUS = 533
EXPECTED_POOL = 287
EXPECTED_STRICT_LENGTHS = Counter({1: 100})
EXPECTED_TOKEN_LENGTHS = Counter({1: 100})
EXPECTED_CATEGORIES = Counter({"NONLEXICAL": 59, "LEXICAL": 27, "FUNCTION": 14})
FIRST_MEMBER = "kaikki_ancient_greek:43342:en--κή-grc-suffix-Wvm-0zVm"
LAST_MEMBER = "kaikki_ancient_greek:54924:en-νῶε-grc-pron-oF7-pU9w"


def inventory_items() -> list[R12.R11.InventoryItem]:
    """Reuse the accepted selector with the round-13 frozen boundaries."""
    R12.EXPECTED_PREVIOUS = EXPECTED_PREVIOUS
    R12.EXPECTED_POOL = EXPECTED_POOL
    R12.EXPECTED_STRICT_LENGTHS = EXPECTED_STRICT_LENGTHS
    R12.EXPECTED_TOKEN_LENGTHS = EXPECTED_TOKEN_LENGTHS
    R12.EXPECTED_CATEGORIES = EXPECTED_CATEGORIES
    R12.FIRST_MEMBER = FIRST_MEMBER
    R12.LAST_MEMBER = LAST_MEMBER
    return R12.inventory_items()


def inventory_card(
    number: int,
    item: R12.R11.InventoryItem,
) -> tuple[str, dict]:
    rendered, record = R12.inventory_card(number, item)
    old = "أول الباقي بعد `00433`"
    if rendered.count(old) != 1:
        raise AssertionError(f"تغير سطر معيار الانتخاب في {number}")
    rendered = rendered.replace(old, "أول الباقي بعد `00533`")
    record = dict(record)
    record["bytes"] = len(rendered.encode("utf-8"))
    if record["bytes"] > R12.R11.R2.MAX_CARD_BYTES:
...
def render_all() -> tuple[list[str], list[dict]]:
    ...
    if Counter(record["verdict"] for record in records) != Counter(
        ...
        raise AssertionError("تغيرت أحكام الجولة الثالثة عشرة")
...
def reading_fragment(
    ...
    if start == first:
        lines += [
            f"<!-- LANE-A-GREEK-ROUND13-BATCH-{batch}:START -->",
            "",
            f"## اليونانية، الجولة الثالثة عشرة: دفعة إتمام الجرد المفتوح {batch} ({DATE})",
            "",
    ...
    for card in selected:
        lines += [card, ""]
    if end == last:
        lines.append(f"<!-- LANE-A-GREEK-ROUND13-BATCH-{batch}:END -->")
    return "\n".join(lines).rstrip()


def stage_fragments() -> Path:
    cards, _ = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round13-"))
    batches = (
...
def emit_staged_patch(stage: Path, start: int) -> str:
    ...
    return R12.R11.R8.emit_append_patch(
        READING,
...
def report_fragment() -> str:
    ...
    return "\n".join([
        ...
        "",
        f"## {DATE}، الجولة الثالثة عشرة، دفعة إتمام الجرد المفتوح 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة الثالثة عشرة، دفعة إتمام الجرد المفتوح 2",
        "",
...
def main() -> int:
    ...
    if args.stage:
        ...
        print(R12.R11.R8.emit_append_patch(
            READING,
