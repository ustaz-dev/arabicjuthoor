#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 42; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative ledger after OPEN-COMP-03020.
...
import harvest_ancient_greek_round41 as R41  # noqa: E402
...
READING, REPORT = R41.READING, R41.REPORT
DATE = "2026-08-28"
FIRST_COMPLETION, LAST_COMPLETION = 3021, 3120
EXPECTED_PREVIOUS = 3020
EXPECTED_POOL = 13_730
REMAINING_POOL = 13_630
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
EXPECTED_STRICT = Counter({2: 100})
EXPECTED_TOKENS = Counter({2: 65, 3: 35})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 100})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:30356:en-κορέω-grc-verb-BsS8-8TO"
LAST_MEMBER = "kaikki_ancient_greek:31164:en-χῶμα-grc-noun-C2sBmuEf"
...
EXPECTED_FAMILY_GROUPS = {
    "ancient_greek:family:67dfa251bc743743d399fb4f": 4,
    "ancient_greek:family:af2087a9ccc3399d50a90d6b": 8,
    "ancient_greek:family:0b36959d0d53ec58baba9586": 2,
    "ancient_greek:family:84f9698b55b1c55bd5f9441a": 2,
    "ancient_greek:family:d295ca1f1a07472deb790a51": 3,
    "ancient_greek:family:bac67378534e5b77653e20a8": 3,
    "ancient_greek:family:4eedcf0f91d459a232dd3a83": 2,
    "ancient_greek:family:423fb524905f465bbebbb50c": 5,
    "ancient_greek:family:cba66d02c84ea37cc1698b72": 12,
    "ancient_greek:family:285956c8efa2bb516cada200": 2,
    "ancient_greek:family:ccf083cb8eebaf29123f4d49": 2,
}


def _configure_renderer() -> None:
    """Bind the stable renderer stack to the round-42 fixed window."""
    values = {
        "DATE": DATE,
    ...
    for name, value in values.items():
        setattr(R41, name, value)
    R41._configure_renderer()


def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    report = REPORT.read_text(encoding="utf-8")
    if "LANE-A DONE41 100 LANE-A-OPEN-COMP-03020" not in report:
        raise AssertionError("خاتمة DONE41 غير مثبتة")
    reading = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND41-CHUNK-50:END -->" not in reading:
        raise AssertionError("الجولة الحادية والأربعون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND42-BATCH-1:START -->" in reading:
        raise AssertionError("بطاقات الجولة الثانية والأربعين موجودة")

    _configure_renderer()
    original_limit = R11.R2.MAX_CARD_BYTES
    R11.R2.MAX_CARD_BYTES = 64 * 1024
    try:
        cards, records, meta = R41.render_all(allow_installed=True)
    finally:
    ...
    for card, record in zip(cards, records, strict=True):
        ...
        if seen_before:
            old_note = "الرسم سبق إتمامه إلى 02920"
            new_note = "الرسم سبق إتمامه إلى 03020"
            if window_count > 1:
                old_note += f" ومكرر في النافذة={window_count}"
                new_note += f" ومكرر في النافذة={window_count}"
        elif window_count == 1:
            old_note = "الرسم جديد إلى 02920"
            new_note = "الرسم جديد إلى 03020"
        else:
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    ...
    return [
        f"<!-- LANE-A-GREEK-ROUND42-BATCH-{batch}:START -->",
        "",
        f"## اليونانية، الجولة الثانية والأربعون: إتمام الجرد المفتوح، الدفعة {batch} ({DATE})",
        "",
...
def report_addition(records: list[dict], meta: dict) -> str:
    ...
    lines = [
        "",
        "<!-- LANE-A-GREEK-ROUND42-REPORT:START -->",
        "",
        f"## {DATE}، الجولة الثانية والأربعون، إتمام الجرد المفتوح، الدفعة 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة الثانية والأربعون، إتمام الجرد المفتوح، الدفعة 2",
        "",
        ...
        f"- انتقال الرتل: أُخذت أول 100 بطاقة باقية من حوض `OPEN-CANDIDATE` بعد `03020`؛ كان الحوض {EXPECTED_POOL} عضوا وبقي بعد النافذة {meta['next_pool_remaining']} عضوا.",
        ...
        "",
        "<!-- LANE-A-GREEK-ROUND42-REPORT:END -->",
        "",
        "LANE-A DONE42 100 LANE-A-OPEN-COMP-03120",
    ]
    report = "\n".join(lines)
    if "—" in report:
        raise AssertionError("شرطة طويلة في تقرير الجولة الثانية والأربعين")
    return report


def stage_patches() -> Path:
    cards, records, meta = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round42-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND41-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND42-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND42-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
...
def verify_installed() -> dict:
    reading = READING.read_text(encoding="utf-8")
    first_marker = "<!-- LANE-A-GREEK-ROUND42-BATCH-1:START -->"
    if first_marker not in reading:
        raise AssertionError("الجولة الحادية والأربعون غير مثبتة")
    section = first_marker + reading.split(first_marker, 1)[1]
    section = section.split("<!-- LANE-A-GREEK-ROUND42-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND42-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND42-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND42-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND42-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE42 100 LANE-A-OPEN-COMP-03120"
    report = REPORT.read_text(encoding="utf-8")
    report_marker = "<!-- LANE-A-GREEK-ROUND42-REPORT:START -->"
