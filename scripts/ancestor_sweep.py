...
**والعربيّةُ ما زالت تنطقُ تلك الحنجريّاتِ كلَّها**: ء ه ح ع غ خ.
...
def main() -> int:
    ...
    for r in rows:
        for layer, form, depth in ancestors_of(r["etymology"])[:3]:
            ...
            if not hits:
                continue
            # الحلقيُّ المستعادُ يشملُ حنجريّاتِ الهندوأوربيّةِ وحلقيّاتِ الساميّة،
            # فكلُّها صوامتُ فقدَها الفرعُ وحفِظَتها العربيّة
