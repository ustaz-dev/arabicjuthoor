...
**والعربيّةُ ما زالت تنطقُ تلك الحنجريّاتِ كلَّها**: ء ه ح ع غ خ. فإن صحَّت دعوى
المشروعِ فالحنجريّةُ المستعادةُ في الفرعِ يجبُ أن تُقابِلَ حلقيًّا عربيًّا مثبتًا،
وهذا مقياسٌ يقبلُ التكذيبَ لا مجرَّدَ تشبيه.
...
# ------------------------------------------------------------------ المروحة
# **الحنجريّاتُ هي بابُ هذا المسحِ كلِّه.** الاستعادةُ تكتبُها h₁ h₂ h₃ وH للمجهولة،
# ولا يُنطَقُ منها شيءٌ في الفروعِ الحيّة. والعربيّةُ تنطقُها كلَّها، فهي الوحيدةُ
# التي يمكنُ أن تشهدَ لها. وقيمُها المستعادةُ في المدرسةِ القياسيّة:
    ...
    for rx in (RX_STAR, RX_SLASH):
        for m in rx.finditer(etymology or ""):
            ...
            if not form or len(form) < 2 or (layer, form) in seen:
                continue
            if not re.search(r"[a-zʰʷ₁₂₃ḱǵćśčǰźžšθðɣ]", form):
                continue
            seen.add((layer, form))
...
def main() -> int:
    ...
    for r in rows:
        for layer, form, depth in ancestors_of(r["etymology"])[:3]:
            ...
            if not hits:
                continue
            laryngeal = any(c in {"h₁", "h₂", "h₃", "H"} for c in sk)
    ...
    # الحنجريّةُ أوّلًا لأنّها الشاهدُ الذي لا يُرى إلّا في العربيّة، ثمّ القِدَمُ،
    # ثمّ الشهادةُ المباشرة
