#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 40; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative ledger after OPEN-COMP-02820.
...
import harvest_ancient_greek_round39 as R39  # noqa: E402


R38, R37, R36, R35, R11 = R39.R38, R39.R37, R39.R36, R39.R35, R39.R11
READING, REPORT = R39.READING, R39.REPORT
DATE = "2026-08-27"
FIRST_COMPLETION, LAST_COMPLETION = 2821, 2920
EXPECTED_PREVIOUS = 2820
EXPECTED_POOL = 13_930
REMAINING_POOL = 13_830
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
EXPECTED_STRICT = Counter({2: 100})
EXPECTED_TOKENS = Counter({3: 90, 4: 9, 2: 1})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 100})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:8470:en-ἕνεκα-grc-postp-W7Kdlp0b"
LAST_MEMBER = "kaikki_ancient_greek:7487:en-κύβη-grc-noun-ny5tM6Nx"
...
EXPECTED_FAMILY_GROUPS = {
    "ancient_greek:family:b7aa9ab265d8bbb5f70d7995": 2,
    "ancient_greek:family:beb23da6ae7a051c50fd6bd8": 2,
    "ancient_greek:family:5efe38f0292143e1f8fde2fd": 4,
    "ancient_greek:family:5ece169afadf2b7ecdbfb926": 2,
    "ancient_greek:family:9f8e631510591826b21dec22": 2,
    "ancient_greek:family:da29cce1fddc08441dae0fa2": 3,
    "ancient_greek:family:277f3a5b6aa0f53162606523": 2,
    "ancient_greek:family:d1b10672fcbab560e807cadf": 2,
    "ancient_greek:family:b4ecb42429137bd12d5fa307": 2,
    "ancient_greek:family:6c667c7acf6fa7573479a825": 2,
    "ancient_greek:family:ece896f1099d91ab70c58678": 2,
    "ancient_greek:family:79d06738a3dff22a1636e153": 2,
    "ancient_greek:family:005ccb7340752656b6028bce": 2,
    "ancient_greek:family:a64175ae0a3a2cd679053557": 2,
    "ancient_greek:family:2ab16ee492ff8f05db90bec8": 2,
}


def _configure_renderer() -> None:
    """Bind the stable renderer stack to the round-40 fixed window."""
    values = {
        "DATE": DATE,
    ...
    for name, value in values.items():
        setattr(R39, name, value)
    R39._configure_renderer()


def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    report = REPORT.read_text(encoding="utf-8")
    if "LANE-A DONE39 100 LANE-A-OPEN-COMP-02820" not in report:
        raise AssertionError("خاتمة DONE39 غير مثبتة")
    reading = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND39-CHUNK-50:END -->" not in reading:
        raise AssertionError("الجولة التاسعة والثلاثون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND40-BATCH-1:START -->" in reading:
        raise AssertionError("بطاقات الجولة الأربعين موجودة")

    _configure_renderer()
    cards, records, meta = R39.render_all(allow_installed=True)
    ...
    for card, record in zip(cards, records, strict=True):
        ...
        if seen_before:
            old_note = "الرسم سبق إتمامه إلى 02720"
            new_note = "الرسم سبق إتمامه إلى 02820"
            if window_count > 1:
                old_note += f" ومكرر في النافذة={window_count}"
                new_note += f" ومكرر في النافذة={window_count}"
        elif window_count == 1:
            old_note = "الرسم جديد إلى 02720"
            new_note = "الرسم جديد إلى 02820"
        else:
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    ...
    return [
        f"<!-- LANE-A-GREEK-ROUND40-BATCH-{batch}:START -->",
        "",
        f"## اليونانية، الجولة الأربعون: إتمام الجرد المفتوح، الدفعة {batch} ({DATE})",
        "",
...
def report_addition(records: list[dict], meta: dict) -> str:
    ...
    lines = [
        "",
        "<!-- LANE-A-GREEK-ROUND40-REPORT:START -->",
        "",
        f"## {DATE}، الجولة الأربعون، إتمام الجرد المفتوح، الدفعة 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة الأربعون، إتمام الجرد المفتوح، الدفعة 2",
        "",
        ...
        f"- انتقال الرتل: أُخذت أول 100 بطاقة باقية من حوض `OPEN-CANDIDATE` بعد `02820`؛ كان الحوض {EXPECTED_POOL} عضوا وبقي بعد النافذة {meta['next_pool_remaining']} عضوا.",
        ...
        "",
        "<!-- LANE-A-GREEK-ROUND40-REPORT:END -->",
        "",
        "LANE-A DONE40 100 LANE-A-OPEN-COMP-02920",
    ]
    report = "\n".join(lines)
    if "—" in report:
        raise AssertionError("شرطة طويلة في تقرير الجولة الأربعين")
    return report


def stage_patches() -> Path:
    cards, records, meta = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round40-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND39-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND40-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND40-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
...
def verify_installed() -> dict:
    reading = READING.read_text(encoding="utf-8")
    first_marker = "<!-- LANE-A-GREEK-ROUND40-BATCH-1:START -->"
    ...
    section = section.split("<!-- LANE-A-GREEK-ROUND40-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND40-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND40-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND40-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND40-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE40 100 LANE-A-OPEN-COMP-02920"
    report = REPORT.read_text(encoding="utf-8")
    report_marker = "<!-- LANE-A-GREEK-ROUND40-REPORT:START -->"
