#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 32; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative inventory after OPEN-COMP-02020
...
import harvest_ancient_greek_round31 as R31  # noqa: E402


R30, R29, R28, R27 = R31.R30, R31.R29, R31.R28, R31.R27
R26, R25 = R31.R26, R31.R25
R20, R19, R18, R17, R16 = R31.R20, R31.R19, R31.R18, R31.R17, R31.R16
READING, REPORT = R31.READING, R31.REPORT
DATE = "2026-08-26"
FIRST_COMPLETION, LAST_COMPLETION = 2021, 2120
EXPECTED_PREVIOUS = 2020
EXPECTED_POOL = 339
REMAINING_AFTER = 239
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
BATCHES = ((2021, 2070), (2071, 2120))
EXPECTED_STRICT = Counter({4: 100})
EXPECTED_TOKENS = Counter({4: 89, 5: 11})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 93, "FUNCTION": 4, "NONLEXICAL": 3})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:26059:en-πυρσός-grc-adj-SL5-zE5r"
LAST_MEMBER = "kaikki_ancient_greek:47437:en-θυμιητός-grc-adj-eb5z6YlU"
...
EXPECTED_FAMILY_GROUPS = {
    ...
    "ancient_greek:family:9661c7a51857d2bfaaf12d40": 2,
    "ancient_greek:family:572a78bda087ec599276d140": 2,
    "ancient_greek:family:efe0c7d58d6db0491a34e6df": 5,
    "ancient_greek:family:58cda282fc91bd9f1aa6f1f0": 2,
    "ancient_greek:family:01a59837f8a5d5384799bd6d": 2,
    "ancient_greek:family:2f16de3eacc6bbe6adb08684": 2,
    "ancient_greek:family:4ca422975a10b6ce45f35187": 2,
    "ancient_greek:family:9d4bc57349f0ced4d7b8b7df": 2,
    "ancient_greek:family:a309e6a3f6774bbb74d1f360": 2,
}


# Rebind the complete historical delegation chain to the round-32 window.
# Historical scripts remain unchanged on disk; only this process is rebound.
for module in (R31, R30, R29, R28, R27, R26, R25, R20, R19, R18, R17):
...
for module in (R31, R30, R29, R28, R27, R26):
...
def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    ...
    if "LANE-A DONE31 100 LANE-A-OPEN-COMP-02020" not in REPORT.read_text(encoding="utf-8"):
        raise AssertionError("خاتمة DONE31 غير مثبتة")
    reading_text = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND31-CHUNK-50:END -->" not in reading_text:
        raise AssertionError("الجولة الحادية والثلاثون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND32-BATCH-1:START -->" in reading_text:
        raise AssertionError("بطاقات الجولة الثانية والثلاثين موجودة")

    cards, records, meta = R31.render_all(allow_installed=True)
    rendered_cards: list[str] = []
    for card, record in zip(cards, records, strict=True):
        card = card.replace("الرسم جديد إلى 01920", "الرسم جديد إلى 02020")
        if "الرسم جديد إلى 01920" in card:
...
def _advance_labels(value: str) -> str:
    """Advance only round labels in a fragment rendered by round 31."""
    replacements = (
        ("LANE-A-GREEK-ROUND31", "LANE-A-GREEK-ROUND32"),
        ("LANE-A DONE31", "LANE-A DONE32"),
        ("الجولة الحادية والثلاثون", "الجولة الثانية والثلاثون"),
        ("الجولة الحادية والثلاثين", "الجولة الثانية والثلاثين"),
    )
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    lines = _advance_labels("\n".join(R31.batch_header(batch, records))).splitlines()
    lines.append("")
    return lines


def report_addition(records: list[dict], meta: dict) -> str:
    return _advance_labels(R31.report_addition(records, meta))


def stage_patches() -> Path:
    cards, records, meta = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round32-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND31-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND32-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND32-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
            patch = R31.R30._anchored_patch(READING, "\n".join(lines), previous_anchor)
...
def verify_installed() -> dict:
    ...
    first_marker = "<!-- LANE-A-GREEK-ROUND32-BATCH-1:START -->"
    ...
    section = section.split("<!-- LANE-A-GREEK-ROUND32-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND32-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND32-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND32-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND32-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE32 100 LANE-A-OPEN-COMP-02120"
    ...
    if (
        ...
        or report.count("<!-- LANE-A-GREEK-ROUND32-REPORT:START -->") != 1
    ):
