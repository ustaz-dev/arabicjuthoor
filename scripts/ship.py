...
def main() -> int:
    ...
    if args.only:
        ...
        # **ولا يُقاسُ المشتقُّ بما تغيّرَ في أثناءِ البناء.** جُرِّبَ ذلك فسقطَ:
        # سجلُّ الاستردادِ كان مُعدَّلًا **قبلَ** بناءِ هذا الشحنِ بفعلِ مسارٍ
        # آخر، فوقعَ في الطرفَينِ فسقطَ من الفرقِ فبقيَ خارجَ الإيداعِ وأسقطَ
        # النشر. **فالمشتقّاتُ تُدرَجُ كلُّها ما دامَت متغيّرة**، لأنّ بوّاباتِ
        # النشرِ تفحصُها كلَّها ولا تسألُ من غيَّرَها.
        DERIVED_OUTSIDE_DATA = ("04-cross-linguistic/recovery-loan-registry.md",)
        dirty = {ln[3:].strip().strip('"') for ln in changed_now()}
        derived = sorted(p for p in dirty
                         if p.startswith("data/") or p in DERIVED_OUTSIDE_DATA)
