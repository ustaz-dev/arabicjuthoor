#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 39; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative ledger after OPEN-COMP-02720.
...
import harvest_ancient_greek_round38 as R38  # noqa: E402


R37, R36, R35, R11 = R38.R37, R38.R36, R38.R35, R38.R11
READING, REPORT = R38.READING, R38.REPORT
DATE = "2026-08-27"
FIRST_COMPLETION, LAST_COMPLETION = 2721, 2820
EXPECTED_PREVIOUS = 2720
EXPECTED_POOL = 14_030
REMAINING_POOL = 13_930
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
EXPECTED_STRICT = Counter({2: 100})
EXPECTED_TOKENS = Counter({3: 92, 4: 6, 2: 2})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 100})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:26709:en-γάθια-grc-noun-ITTKpHaQ"
LAST_MEMBER = "kaikki_ancient_greek:25994:en-ὅνδε-grc-pron-8LS~2HU7"
EXPECTED_PRIOR_SURFACES = {
    "LANE-A-OPEN-COMP-02787": "ἅλιος",
    "LANE-A-OPEN-COMP-02788": "ἅλιος",
}
...
EXPECTED_FAMILY_GROUPS = {
    "ancient_greek:family:30177bb9d562518ebfa53d1e": 2,
    "ancient_greek:family:9e259a0e24ed709f14e224d2": 2,
    "ancient_greek:family:fbf0223a55fb70079475010b": 2,
    "ancient_greek:family:53906a4f15dcc9b004ae75d3": 3,
    "ancient_greek:family:350850795ad750eff751fd36": 3,
    "ancient_greek:family:d7825c79d6981e0d66e33871": 2,
    "ancient_greek:family:b7e636d2c919111d3119b4c7": 3,
    "ancient_greek:family:2b6ca1f1b165961f3481270c": 2,
    "ancient_greek:family:94af5fb926daec829ff506c8": 2,
    "ancient_greek:family:af7b423a56744596c4cbbec4": 2,
    "ancient_greek:family:d1a8df00bcc42d6ced3b79fc": 2,
    "ancient_greek:family:b8a6924d9c11b27ef23add6b": 7,
    "ancient_greek:family:bd2726f0dd332164bc2fce66": 2,
    "ancient_greek:family:0ce3121a353a061eb60c820a": 2,
    "ancient_greek:family:2204398a2322e6ba8cbc1ae2": 2,
    "ancient_greek:family:88404c9ed5088ce8e70889a8": 4,
    "ancient_greek:family:720b40de541f9aac783ce50d": 2,
    "ancient_greek:family:eb036d36087395921cdf6b4f": 3,
}


def _configure_renderer() -> None:
    """Bind the stable renderer stack to the round-39 fixed window."""
    values = {
        "DATE": DATE,
    ...
    for name, value in values.items():
        setattr(R38, name, value)
    R38._configure_renderer()


def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    report = REPORT.read_text(encoding="utf-8")
    if "LANE-A DONE38 100 LANE-A-OPEN-COMP-02720" not in report:
        raise AssertionError("خاتمة DONE38 غير مثبتة")
    reading = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND38-CHUNK-50:END -->" not in reading:
        raise AssertionError("الجولة الثامنة والثلاثون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND39-BATCH-1:START -->" in reading:
        raise AssertionError("بطاقات الجولة التاسعة والثلاثين موجودة")

    _configure_renderer()
    cards, records, meta = R38.render_all(allow_installed=True)
    ...
    for card, record in zip(cards, records, strict=True):
        ...
        if seen_before:
            old_note = "الرسم سبق إتمامه إلى 02620"
            new_note = "الرسم سبق إتمامه إلى 02720"
            if window_count > 1:
                old_note += f" ومكرر في النافذة={window_count}"
                new_note += f" ومكرر في النافذة={window_count}"
        elif window_count == 1:
            old_note = "الرسم جديد إلى 02620"
            new_note = "الرسم جديد إلى 02720"
        else:
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    ...
    return [
        f"<!-- LANE-A-GREEK-ROUND39-BATCH-{batch}:START -->",
        "",
        f"## اليونانية، الجولة التاسعة والثلاثون: إتمام الجرد المفتوح، الدفعة {batch} ({DATE})",
        "",
...
def report_addition(records: list[dict], meta: dict) -> str:
    ...
    lines = [
        "",
        "<!-- LANE-A-GREEK-ROUND39-REPORT:START -->",
        "",
        f"## {DATE}، الجولة التاسعة والثلاثون، إتمام الجرد المفتوح، الدفعة 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة التاسعة والثلاثون، إتمام الجرد المفتوح، الدفعة 2",
        "",
        ...
        f"- انتقال الرتل: أُخذت أول 100 بطاقة باقية من حوض `OPEN-CANDIDATE` بعد `02720`؛ كان الحوض {EXPECTED_POOL} عضوا وبقي بعد النافذة {meta['next_pool_remaining']} عضوا.",
        ...
        "",
        "<!-- LANE-A-GREEK-ROUND39-REPORT:END -->",
        "",
        "LANE-A DONE39 100 LANE-A-OPEN-COMP-02820",
    ]
    report = "\n".join(lines)
    if "—" in report:
        raise AssertionError("شرطة طويلة في تقرير الجولة التاسعة والثلاثين")
    return report


def stage_patches() -> Path:
    cards, records, meta = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round39-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND38-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND39-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND39-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
...
def verify_installed() -> dict:
    reading = READING.read_text(encoding="utf-8")
    first_marker = "<!-- LANE-A-GREEK-ROUND39-BATCH-1:START -->"
    if first_marker not in reading:
        raise AssertionError("الجولة التاسعة والثلاثون غير مثبتة")
    section = first_marker + reading.split(first_marker, 1)[1]
    section = section.split("<!-- LANE-A-GREEK-ROUND39-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND39-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND39-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND39-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND39-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE39 100 LANE-A-OPEN-COMP-02820"
    report = REPORT.read_text(encoding="utf-8")
    report_marker = "<!-- LANE-A-GREEK-ROUND39-REPORT:START -->"
