# -*- coding: utf-8 -*-
"""الجولة التاسعة عشرة للمسار B: دفعتا إتمام للمفتوح الفارسي القصير."""
...
import harvest_persian_round18 as PREVIOUS


BASE = PREVIOUS.BASE
FORMATTER = PREVIOUS.PREVIOUS
DECIDER = PREVIOUS.PREVIOUS.PREVIOUS
FIRST_COMPLETION = 1341
LAST_COMPLETION = 1410
BATCH_SIZE = 35
MARKER = "LANE-B-PERSIAN-ROUND19-2026-08-18"
FIRST_WORD = "له"
LAST_WORD = "ـیسم"
FIRST_SOURCE_INDEX = 8603
LAST_SOURCE_INDEX = 12136
EXPECTED_AVAILABLE = 201
...
def available_cards(
    ...
    """يأخذ النافذة التالية من ذيل الحوض الحي بعد الجولة الثامنة عشرة."""
...
def validate_selection(
    ...
    if len(all_available) != EXPECTED_AVAILABLE:
        raise AssertionError(
            f"تغير عدد الحوض القصير الحي في مطلع الجولة التاسعة عشرة: {len(all_available)}"
        )
    if len(cards) != expected_count:
        raise AssertionError(
            f"عُثر على {len(cards)} بطاقة في نافذة الجولة التاسعة عشرة؛ يلزم 70"
        )
    if cards[0].word != FIRST_WORD or cards[-1].word != LAST_WORD:
        raise AssertionError("تغير طرفا نافذة الجولة التاسعة عشرة")
    if (
        cards[0].source_index != FIRST_SOURCE_INDEX
        or cards[-1].source_index != LAST_SOURCE_INDEX
    ):
        raise AssertionError("تغير طرفا ترتيب المصدر في الجولة التاسعة عشرة")
    member_ids = [card.member_id for card in cards]
    if len(member_ids) != len(set(member_ids)):
        raise AssertionError("تكرر عضو في نافذة الجولة التاسعة عشرة")
...
def validate_new_text(
    ...
    if len(cards) != expected_count or len(texts) != expected_count:
        raise AssertionError("لم تكتمل نافذة الجولة التاسعة عشرة إلى 70 بطاقة")
    if len(decisions) != expected_count:
        raise AssertionError("عدد أحكام الجولة التاسعة عشرة غير مكتمل")
    headings = [
        int(value)
        for text in texts
    ...
    if any(item.verdict in POSITIVE_VERDICTS for item in decisions):
        raise AssertionError("دخل حكم موجب غير مثبت في الجولة التاسعة عشرة")
...
def report_section(
    ...
    for batch in range(2):
        ...
        sections.extend(
            [
                f"## الجولة التاسعة عشرة، دفعة إتمام المفتوح القصير رقم {batch + 1}",
                "",
...
def validate_existing(reading_text: str, report_text: str) -> None:
    ...
    if not reading_match:
        raise AssertionError("محضر الجولة التاسعة عشرة موجود بلا مقطع القراءة")
    ids = [
        int(value)
        for value in re.findall(
            r"^### WO-B-OPEN-COMP-(\d+):",
    ...
    if ids != list(range(FIRST_COMPLETION, LAST_COMPLETION + 1)):
        raise AssertionError("مقطع الجولة التاسعة عشرة الموجود غير متصل")
    ...
    expected = "LANE-B DONE19 70 WO-B-OPEN-COMP-01410"
    if not report_text.rstrip().endswith(expected):
        raise AssertionError("سطر DONE19 ليس خاتمة التقرير")
...
def build_round(
    ...
    decisions = [DECIDER.select_decision(card, sense_map) for card in cards]
    texts = [
        FORMATTER.fit_card(number, card, decision, sense_map)
...
def main() -> int:
    ...
    if MARKER in reading_text or MARKER in report_text:
        if MARKER not in reading_text or MARKER not in report_text:
            raise AssertionError("الجولة التاسعة عشرة مكتوبة جزئيا")
        validate_existing(reading_text, report_text)
        print("ROUND19 ALREADY PRESENT AND VALID")
        return 0
    ...
    reading_append = (
        f"\n\n<!-- {MARKER}:START -->\n\n"
        "## الجولة التاسعة عشرة: مواصلة المفتوح الفارسي القصير (2026-08-18)\n\n"
        "- بيان النطاق: بعد قبول الجولة الثامنة عشرة وتسويتها استمر جرد بطاقات العضو المباشر من ذيل المصدر القصير الحي؛ أُخذت 70 بطاقة مرتبة بموضع المصدر، من `له` إلى `ـیسم`، في دفعتين 35 و35، بلا توسعة هيكل.\n\n"
        + "\n".join(texts[:BATCH_SIZE])
        + "\n## الدفعة الثانية، الإتمامات 01376 إلى 01410\n\n"
        + "\n".join(texts[BATCH_SIZE:])
        + f"\n<!-- {MARKER}:END -->\n"
    )
    report_append = (
    ...
    print("LANE-B DONE19 70 WO-B-OPEN-COMP-01410")
    return 0
