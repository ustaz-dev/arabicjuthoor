#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 43; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative ledger after OPEN-COMP-03120.
...
import harvest_ancient_greek_round42 as R42  # noqa: E402
...
READING, REPORT = R42.READING, R42.REPORT
DATE = "2026-08-28"
FIRST_COMPLETION, LAST_COMPLETION = 3121, 3220
EXPECTED_PREVIOUS = 3120
EXPECTED_POOL = 13_630
REMAINING_POOL = 13_530
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
EXPECTED_STRICT = Counter({2: 100})
EXPECTED_TOKENS = Counter({2: 94, 3: 6})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 100})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:53578:en-ἄχμα-grc-noun-MyJTLYWJ"
LAST_MEMBER = "kaikki_ancient_greek:10624:en-λύμη-grc-noun-cZJixTp0"
EXPECTED_PRIOR_SURFACES = {
    "LANE-A-OPEN-COMP-03164": "χάος",
}
...
EXPECTED_FAMILY_GROUPS = {
    "ancient_greek:family:e8ec26cdf970eac115f70447": 10,
    "ancient_greek:family:729f29dcb9cee58ded8c5d79": 10,
    "ancient_greek:family:ccb826fc7809c14603581230": 7,
    "ancient_greek:family:d1981e91684815ceaa4f6a0f": 2,
    "ancient_greek:family:b68fdc981088e22a03e4bf60": 3,
    "ancient_greek:family:a93ed98c390f64c0b9d1f13f": 5,
    "ancient_greek:family:8d628e96f6fe278a94ea35ab": 2,
    "ancient_greek:family:7143bb23842971641159196d": 2,
}

def _configure_renderer() -> None:
    """Bind the stable renderer stack to the round-43 fixed window."""
    values = {
        "DATE": DATE,
    ...
    for name, value in values.items():
        setattr(R42, name, value)
    R42._configure_renderer()


def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    report = REPORT.read_text(encoding="utf-8")
    if "LANE-A DONE42 100 LANE-A-OPEN-COMP-03120" not in report:
        raise AssertionError("خاتمة DONE42 غير مثبتة")
    reading = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND42-CHUNK-50:END -->" not in reading:
        raise AssertionError("الجولة الثانية والأربعون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND43-BATCH-1:START -->" in reading:
        raise AssertionError("بطاقات الجولة الثالثة والأربعين موجودة")

    _configure_renderer()
    original_limit = R11.R2.MAX_CARD_BYTES
    R11.R2.MAX_CARD_BYTES = 64 * 1024
    try:
        cards, records, meta = R42.render_all(allow_installed=True)
    finally:
    ...
    for card, record in zip(cards, records, strict=True):
        ...
        if seen_before:
            old_note = "الرسم سبق إتمامه إلى 03020"
            new_note = "الرسم سبق إتمامه إلى 03120"
            if window_count > 1:
                old_note += f" ومكرر في النافذة={window_count}"
                new_note += f" ومكرر في النافذة={window_count}"
        elif window_count == 1:
            old_note = "الرسم جديد إلى 03020"
            new_note = "الرسم جديد إلى 03120"
        else:
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    ...
    return [
        f"<!-- LANE-A-GREEK-ROUND43-BATCH-{batch}:START -->",
        "",
        f"## اليونانية، الجولة الثالثة والأربعون: إتمام الجرد المفتوح، الدفعة {batch} ({DATE})",
        "",
...
def report_addition(records: list[dict], meta: dict) -> str:
    ...
    lines = [
        "",
        "<!-- LANE-A-GREEK-ROUND43-REPORT:START -->",
        "",
        f"## {DATE}، الجولة الثالثة والأربعون، إتمام الجرد المفتوح، الدفعة 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة الثالثة والأربعون، إتمام الجرد المفتوح، الدفعة 2",
        "",
        ...
        "",
        "## حصيلة الجولة الثالثة والأربعين",
        "",
        "- مجموع البطاقات المكتوبة: 100؛ دفعتان من 50 بطاقة.",
        f"- انتقال الرتل: أُخذت أول 100 بطاقة باقية من حوض `OPEN-CANDIDATE` بعد `03120`؛ كان الحوض {EXPECTED_POOL} عضوا وبقي بعد النافذة {meta['next_pool_remaining']} عضوا.",
        ...
        f"- ضبط الأرجل الثلاث: موجب مكتمل=0؛ حكم شريف مسمى السبب={meta['honourable_reasoned']}؛ حملت {sum(record['verdict'] == 'OPEN-CANDIDATE' for record in records)} بطاقة حكما مفتوحا لغياب مدار بشري محدود، وحملت {len(source_gaps)} بطاقة `SOURCE-GAP` لغياب شاهد مصدري صالح.",
        source_gate,
        ...
        "",
        "<!-- LANE-A-GREEK-ROUND43-REPORT:END -->",
        "",
        "LANE-A DONE43 100 LANE-A-OPEN-COMP-03220",
    ]
    report = "\n".join(lines)
    if "—" in report:
        raise AssertionError("شرطة طويلة في تقرير الجولة الثالثة والأربعين")
    return report


def stage_patches() -> Path:
    cards, records, meta = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round43-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND42-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND43-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND43-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
    ...
    (stage / "report.patch").write_text(
        R35.R34.R33.R30._anchored_patch(
            ...
            "LANE-A DONE42 100 LANE-A-OPEN-COMP-03120",
        ),
        encoding="utf-8",
        newline="\n",
    )
...
def verify_installed() -> dict:
    reading = READING.read_text(encoding="utf-8")
    first_marker = "<!-- LANE-A-GREEK-ROUND43-BATCH-1:START -->"
    if first_marker not in reading:
        raise AssertionError("الجولة الثانية والأربعون غير مثبتة")
    section = first_marker + reading.split(first_marker, 1)[1]
    section = section.split("<!-- LANE-A-GREEK-ROUND43-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND43-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND43-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND43-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND43-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE43 100 LANE-A-OPEN-COMP-03220"
    report = REPORT.read_text(encoding="utf-8")
    report_marker = "<!-- LANE-A-GREEK-ROUND43-REPORT:START -->"
