...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="Require 42 rows and pass the ten manual fixtures.")
    ...
    if args.check:
        ...
        if len(rules) != 42:
            failures.append(f"expected 42 operative rows, got {len(rules)}")
        if failures:
            for failure in failures:
                print(f"FAIL: {failure}")
            return 1
        print("network compiler: PASS (42 rows; 10 manual fixtures)")
    return 0
