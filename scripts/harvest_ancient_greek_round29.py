#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 29; emit patches only, never commit, ship, or use git.

Continue the Ancient Greek open comparative inventory after OPEN-COMP-01720
...
import harvest_ancient_greek_round28 as R28  # noqa: E402


R27, R26, R25 = R28.R27, R28.R26, R28.R25
R20, R19, R18, R17, R16 = R28.R20, R28.R19, R28.R18, R28.R17, R28.R16
READING, REPORT = R28.READING, R28.REPORT
DATE = "2026-08-26"
FIRST_COMPLETION, LAST_COMPLETION = 1721, 1820
EXPECTED_PREVIOUS = 1720
EXPECTED_POOL = 639
REMAINING_AFTER = 539
BATCH_SIZE = 50
CARD_COUNT = 100
CHUNK_SIZE = 2
BATCHES = ((1721, 1770), (1771, 1820))
EXPECTED_STRICT = Counter({3: 98, 4: 2})
EXPECTED_TOKENS = Counter({3: 93, 4: 7})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 65, "NONLEXICAL": 28, "FUNCTION": 7})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 99, "SOURCE-GAP": 1})
FIRST_MEMBER = "kaikki_ancient_greek:28984:en-ὄρνυτ'-grc-verb-vKtrhL2w"
LAST_MEMBER = "kaikki_ancient_greek:52350:en-βλέφαρ'-grc-noun-nDGkmpV9"
...
EXPECTED_FAMILY_GROUPS = {
    "ancient_greek:family:ff53f5ec7babbdbc81cde564": 2,
    "ancient_greek:family:b8501edb93ee9359fb60f1e7": 2,
    "ancient_greek:family:38e5dd90afd94dded938ff6e": 4,
    "ancient_greek:family:197e8cbb0b9bbe9c5d1a5e6a": 2,
    "ancient_greek:family:486d62e96c4b5ed75334702d": 2,
}


# Rebind the historical delegating chain to the fixed round-29 window. No
# historical script is changed on disk; only this process receives the new
# invariants.
for module in (R28, R27, R26, R25, R20, R19, R18, R17):
...
def render_all(*, allow_installed: bool = False) -> tuple[list[str], list[dict], dict]:
    ...
    if "LANE-A DONE28 100 LANE-A-OPEN-COMP-01720" not in REPORT.read_text(encoding="utf-8"):
        raise AssertionError("خاتمة DONE28 غير مثبتة")
    reading_text = READING.read_text(encoding="utf-8")
    if "<!-- LANE-A-GREEK-ROUND28-CHUNK-50:END -->" not in reading_text:
        raise AssertionError("الجولة الثامنة والعشرون غير مثبتة")
    if not allow_installed and "<!-- LANE-A-GREEK-ROUND29-BATCH-1:START -->" in reading_text:
        raise AssertionError("بطاقات الجولة التاسعة والعشرين موجودة")

    cards, records, meta = R28.render_all(allow_installed=True)
    rendered_cards: list[str] = []
    for card, record in zip(cards, records, strict=True):
        card = card.replace("الرسم جديد إلى 01620", "الرسم جديد إلى 01720")
        if "الرسم جديد إلى 01620" in card:
...
def batch_header(batch: int, records: list[dict]) -> list[str]:
    ...
    return [
        f"<!-- LANE-A-GREEK-ROUND29-BATCH-{batch}:START -->",
        "",
        f"## اليونانية، الجولة التاسعة والعشرون: إتمام الجرد المفتوح، الدفعة {batch} ({DATE})",
        "",
...
def report_addition(records: list[dict], meta: dict) -> str:
    ...
    return "\n".join([
        ...
        f"## {DATE}، الجولة التاسعة والعشرون، إتمام الجرد المفتوح، الدفعة 1", "",
        ...
        f"## {DATE}، الجولة التاسعة والعشرون، إتمام الجرد المفتوح، الدفعة 2", "",
        ...
        f"- ضبط الأرجل الثلاث: موجب مكتمل=0؛ حكم شريف مسمى السبب={meta['honourable_reasoned']}؛ حملت 99 بطاقة `OPEN-CANDIDATE` لغياب مدار بشري محدود، وحملت بطاقة واحدة `SOURCE-GAP` لغياب شاهد مصدري صالح.",
        f"- بوابة المصدر: بطاقة `SOURCE-GAP` هي {source_gap_words}؛ بقيت مفتوحة ولم يصنع غياب المصدر صلة.",
        ...
        f"LANE-A DONE29 {len(records)} {records[-1]['completion_id']}",
    ])
...
def stage_patches() -> Path:
    ...
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round29-"))
    previous_anchor = "<!-- LANE-A-GREEK-ROUND28-CHUNK-50:END -->"
    chunk_number = 0
    for batch in (1, 2):
        ...
        for offset in range(0, BATCH_SIZE, CHUNK_SIZE):
            ...
            if offset + CHUNK_SIZE == BATCH_SIZE:
                lines += [f"<!-- LANE-A-GREEK-ROUND29-BATCH-{batch}:END -->", ""]
            marker = f"<!-- LANE-A-GREEK-ROUND29-CHUNK-{chunk_number:02d}:END -->"
            lines.append(marker)
...
def verify_installed() -> dict:
    ...
    first_marker = "<!-- LANE-A-GREEK-ROUND29-BATCH-1:START -->"
    ...
    section = section.split("<!-- LANE-A-GREEK-ROUND29-CHUNK-50:END -->", 1)[0]
    ids = [
        int(value)
    ...
    batch_counts = [
        len(re.findall(
            r"^### LANE-A-OPEN-COMP-",
            section.split(f"<!-- LANE-A-GREEK-ROUND29-BATCH-{batch}:START -->", 1)[1]
                   .split(f"<!-- LANE-A-GREEK-ROUND29-BATCH-{batch}:END -->", 1)[0],
            re.MULTILINE,
        ))
        for batch in (1, 2)
    ]
    cards = re.findall(
        r"(?ms)^### LANE-A-OPEN-COMP-.*?(?=^### LANE-A-OPEN-COMP-|^<!-- LANE-A-GREEK-ROUND29-(?:BATCH|CHUNK)-)",
        section,
    )
    ...
    markers = re.findall(r"<!-- LANE-A-GREEK-ROUND29-CHUNK-\d{2}:END -->", reading)
    ...
    done = "LANE-A DONE29 100 LANE-A-OPEN-COMP-01820"
    ...
    if (
        ...
        or report.count("<!-- LANE-A-GREEK-ROUND29-REPORT:START -->") != 1
    ):
