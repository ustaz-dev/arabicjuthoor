#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Render lane A round 20 without committing, shipping, or invoking git.

Round 19 ended at OPEN-COMP-01220. This round reads the next one hundred
...
import harvest_ancient_greek_round19 as R19  # noqa: E402


READING, REPORT = R19.READING, R19.REPORT
DATE = "2026-08-18"
FIRST_COMPLETION, LAST_COMPLETION = 1221, 1320
EXPECTED_PREVIOUS = 1220
EXPECTED_POOL = 1_139
BATCH_SIZE = 50
BATCHES = ((1221, 1270), (1271, 1320))
EXPECTED_STRICT = Counter({2: 100})
EXPECTED_TOKENS = Counter({2: 93, 3: 7})
EXPECTED_CATEGORIES = Counter({"LEXICAL": 53, "NONLEXICAL": 39, "FUNCTION": 8})
EXPECTED_VERDICTS = Counter({"OPEN-CANDIDATE": 100})
FIRST_MEMBER = "kaikki_ancient_greek:46321:en-φάρω-grc-verb-but3D3mK"
LAST_MEMBER = "kaikki_ancient_greek:43486:en-τοιοῦδ'-grc-adj-VklE-tkq"


# Bind every delegating layer to the round-20 window. Earlier scripts remain
# unchanged on disk; only this process receives the new invariants.
for module in (R19, R19.R18, R19.R18.R17):
...
R19.R18.R17.R16.DATE = DATE


def render_all() -> tuple[list[str], list[dict]]:
    """Render and validate the fixed round-20 window."""
    cards, records = R19.render_all()
    if len(cards) != 100 or len(records) != 100:
        raise AssertionError("تغير حجم نافذة الجولة العشرين")
    expected = [
...
def reading_fragment(start: int, count: int, cards: list[str] | None = None) -> str:
    ...
    if start == first:
        lines += [
            f"<!-- LANE-A-GREEK-ROUND20-BATCH-{batch}:START -->",
            "",
            f"## اليونانية، الجولة العشرون: دفعة إتمام الجرد المفتوح {batch} ({DATE})",
            "",
    ...
    for card in selected:
        lines += [card, ""]
    if end == last:
        lines.append(f"<!-- LANE-A-GREEK-ROUND20-BATCH-{batch}:END -->")
    return "\n".join(lines).rstrip()


def stage_fragments() -> Path:
    cards, _ = render_all()
    stage = Path(tempfile.mkdtemp(prefix="lane-a-round20-"))
...
def append_patch(path: Path, fragment: str, marker: str) -> str:
    return R19.append_patch(path, fragment, marker)
...
def report_fragment(records: list[dict] | None = None) -> str:
    ...
    return "\n".join([
        ...
        "",
        f"## {DATE}، الجولة العشرون، دفعة إتمام الجرد المفتوح 1",
        "",
        ...
        "",
        f"## {DATE}، الجولة العشرون، دفعة إتمام الجرد المفتوح 2",
        "",
...
def main() -> int:
    ...
    if args.stage:
        ...
        print(append_patch(REPORT, report_fragment(), "LANE-A DONE20"), end="")
    else:
        _, records = render_all()
