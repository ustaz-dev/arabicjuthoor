# -*- coding: utf-8 -*-
"""الجولة الرابعة عشرة للمسار B: دفعتا إتمام للمفتوح الفارسي القصير."""
...
import harvest_persian_round13 as PREVIOUS


BASE = PREVIOUS.BASE
FIRST_COMPLETION = 991
LAST_COMPLETION = 1060
BATCH_SIZE = 35
MARKER = "LANE-B-PERSIAN-ROUND14-2026-08-17"
FIRST_WORD = "شلف"
LAST_WORD = "رینیت"
POSITIVE_VERDICTS = {
    "ROOT-TRACE",
...
def select_decision(card: BASE.SourceCard, sense_map: dict[str, list[dict]]) -> BASE.Decision:
    """يثبت اللقاءين المباشرين اللذين بقيا بعد قراءة المروحة والشواهد."""
    decisions = {
        "طبله": BASE.Decision(
            candidate="طبل",
            verdict="ROOT-TRACE",
            state="READY",
            orbit=(
                "الفرع يسمّي الطبلة صينية مستديرة، ولسان العرب وتاج العروس "
                "يسمّيان الطبلة شيئا من خشب تتخذه النساء، ويسمّيان الطبل سلة "
                "الطعام كالخوان؛ فالمدار صينية الطعام وخوانه نفسهما مباشرة."
            ),
        ...
        "لاک": BASE.Decision(
            ...
            state="READY",
            orbit=(
                "الفرع يسمّي اللك طلاء الأظافر وينص خبر أصله على lac، والصحاح "
                "وأساس البلاغة يسمّيان اللك صبغا أحمر يصبغ به الجلد؛ فالمدار "
                "مادة الطلاء والصبغ مباشرة، مع اختلاف موضع الاستعمال."
            ),
            obstacle="اكتملت أرجل الصوت والحدث والمعنى بشاهدين عربيين مستقلين.",
        ),
    }
...
def selected_witnesses(
    ...
    targets = {
        ...
        "لكك": (
            (
                "al_sihah",
                ("شى أحمر يُصبغُ به", "شيء أحمر يصبغ به"),
                45,
            ),
            (
                "asas_al_balagha",
                ("صبغ الجلد بالّلكّ", "صبغ الجلد باللك"),
                35,
            ),
        ),
    }
...
def formatted_route(card: BASE.SourceCard, decision: BASE.Decision) -> str:
    if card.word == "طبله" and decision.candidate == "طبل":
        return (
            "الرصف المفحوص: ط↔ط=`IDN-18`، ب↔ب=`IDN-05`، ل↔ل=`IDN-04`؛ "
            "المرشح داخل المروحة."
        )
    if card.word == "لاک" and decision.candidate == "لكك":
        return (
            "الرصف المفحوص: ل↔ل=`IDN-04`، ک↔ك=`IDN-13`؛ باب المضاعف "
            "يكرر الصامت الأخير، ويعضده أصل `lakkhā` المنقول في بطاقة الفرع؛ "
            "المرشح داخل المروحة."
        )
...
def validate_new_text(
    ...
    if len(cards) != expected_count or len(texts) != expected_count:
        raise AssertionError("لم تكتمل نافذة الجولة الرابعة عشرة إلى 70 بطاقة")
    if len(decisions) != expected_count:
        raise AssertionError("عدد أحكام الجولة الرابعة عشرة غير مكتمل")
    if cards[0].word != FIRST_WORD or cards[-1].word != LAST_WORD:
        raise AssertionError("تغير طرفا نافذة الجولة الرابعة عشرة")
    ...
    positive_specs = {
        1020: ("`طبل`", "لسان العرب لابن منظور", "تاج العروس لمرتضى الزبيدي"),
        1044: ("`لكك`", "تاج اللغة وصحاح العربية للجوهري", "أساس البلاغة للزمخشري"),
    }
    ...
    if "سَلَّة الطعام" not in texts[1020 - FIRST_COMPLETION]:
        raise AssertionError("لم يُنقل موضع سلة الطعام في شاهد بطاقة 01020")
    if "كالخُوانِ" not in texts[1020 - FIRST_COMPLETION]:
        raise AssertionError("لم يُنقل موضع الخوان في شاهد بطاقة 01020")
...
def report_section(
    ...
    for batch in range(2):
        ...
        sections.extend(
            [
                f"## الجولة الرابعة عشرة، دفعة إتمام المفتوح القصير رقم {batch + 1}",
                "",
...
def validate_existing(reading_text: str, report_text: str) -> None:
    ...
    if not reading_match:
        raise AssertionError("محضر الجولة الرابعة عشرة موجود بلا مقطع القراءة")
    ids = [
        int(value)
    ...
    if ids != list(range(FIRST_COMPLETION, LAST_COMPLETION + 1)):
        raise AssertionError("مقطع الجولة الرابعة عشرة الموجود غير متصل")
    ...
    expected = "LANE-B DONE14 70 WO-B-OPEN-COMP-01060"
    if not report_text.rstrip().endswith(expected):
        raise AssertionError("سطر DONE14 ليس خاتمة التقرير")
...
def main() -> int:
    ...
    if MARKER in reading_text or MARKER in report_text:
        if MARKER not in reading_text or MARKER not in report_text:
            raise AssertionError("الجولة الرابعة عشرة مكتوبة جزئيا")
        validate_existing(reading_text, report_text)
        print("ROUND14 ALREADY PRESENT AND VALID")
        return 0
    ...
    texts = [
        PREVIOUS.PREVIOUS.PREVIOUS.fit_card(number, card, decision, sense_map)
    ...
    reading_append = (
        f"\n\n<!-- {MARKER}:START -->\n\n"
        "## الجولة الرابعة عشرة: استمرار إتمام المفتوح الفارسي القصير (2026-08-17)\n\n"
        "- بيان النطاق: بعد قبول الجولة الثالثة عشرة وتسويتها أعيد حساب الحالة الحية. بقي 248 عضوا قصيرا غير مكرر ذا مروحة غير فارغة؛ أخذت أول 70 في ترتيب المصدر، من `شلف` إلى `رینیت`، في دفعتين 35 و35.\n\n"
        + "\n".join(texts[:BATCH_SIZE])
        + "\n## الدفعة الثانية، الإتمامات 01026 إلى 01060\n\n"
        + "\n".join(texts[BATCH_SIZE:])
        + f"\n<!-- {MARKER}:END -->\n"
    )
